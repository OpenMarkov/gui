...
public class GUIDefaultStates extends DefaultStates{
	...
	public static String getString(String element) {
		...
		String newKey = "defaultStates." + element + ".Text";
			if (stringResource.getString(newKey).equals(">>> "+newKey+" <<<"))
				return element;
			else
				return stringResource.getString(newKey);
	 ...
	 * 
	 * @param elements
	 *            states whose language-dependent strings are concatenated.
	...
	public static String getString(String[] elements) {
		...
		if ((l == 0) || ((l == 1) && elements[0].equals("nonamed"))) {
			result = stringResource.getString("defaultStates.others.Text");
		} else {
			l--;
			for (i = 0; i < l; i++) {
	 ...
	 * 
	 * @param languageDependentString
	 *            language-dependent string corresponding to the state.
	 * @return the name of the state or the string passed as parameter if any
	 *         state corresponds to the language-dependent string.
	 */
	public static String getStringLanguageDependent(
		...
		while (!found && (i1 < l1)) {
			...
			l2 = elements.size();
			while (!found && (i2 < l2)) {
				if (languageDependentString.equals(
						getString(elements.get(i2)))) {
	 ...
	 * 
	 * @param languageDependentStrings
	 *            language-dependent strings corresponding to the states.
	 * @return the name of each state if its language-dependent string is
	 *         correct or the string that is passed as parameter.
	 */
	public static String[] getStringsLanguageDependent(
			String[] languageDependentStrings) {
		...
		for (i = 0; i < l; i++) {
			elements[i] =
				getStringLanguageDependent(languageDependentStrings[i]);
		}
		return elements;
	}
