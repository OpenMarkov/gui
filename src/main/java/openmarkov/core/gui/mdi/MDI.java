...
//import openmarkov.gui.resources.OpenMarkovLogoIcon;
 ...
 */
public class MDI extends JPanel implements FrameTitleListener,
	 ...
	 * 
	 * @param menu
	 *            menu where put the MDI options.
	 */
	...
	public void createNewFrame(FrameContentPanel newContentPanel)
		...
		frame =
			(InternalFrame) desktopPane.createNewInternalFrame(newContentPanel);
		...
		try {
			...
			throw new UnsupportedOperationException(stringResource
				.getString("SelectionVetoed.Text.Label"));
		}
	...
	public FrameContentPanel getCurrentPanel() {
		...
		return (FrameContentPanel) desktopPane.getSelectedFrame()
			.getContentPane();
	 ...
	 * 
	 * @param l
	 *            listener to set as the registered listener.
	 */
	 ...
	 * 
	 * @param frame
	 *            frame that is going to be closed.
	...
	private boolean notifyFrameClosing(JInternalFrame frame) {
		...
		for (MDIListener listener : mdiListeners) {
			canBeClosed =
				listener.frameClosing((FrameContentPanel) frame
					.getContentPane());
		}
	...
	public void actionPerformed(ActionEvent e) {
		...
		try {
			if (actionCommand.equals(MDIMenu.WINDOW_MINIMIZEALL_MENUITEM)) {
				...
				desktopPane.selectFrame(mdiMenu
					.getPanelByMenuItem((JCheckBoxMenuItem) e.getSource()));
			}
		} catch (UnsupportedOperationException exc) {
			System.err.println(exc.getMessage());
		}
