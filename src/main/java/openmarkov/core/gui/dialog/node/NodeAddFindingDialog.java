...
/**
 * Dialog box to add a finding in a node. The result of using this class
 * is equivalent to a double-click on a node's state when the working mode
 * is 'Inference mode'
 * 
 * @author asaez
 * @version 1.0
...
public class NodeAddFindingDialog extends JDialog {
	...
	public NodeAddFindingDialog(Window owner, VisualNode visualNode, Graphics2D g, EditorPanel editorPanel) {
		...
		JButton okButton = new JButton(stringResource.
				getString("NodeAddFindingDialog.jButtonOK.Label"));		
		JButton cancelButton = new JButton(stringResource.
				getString("NodeAddFindingDialog.jButtonCancel.Label"));
		...
		if (visualNode.getInnerBox() instanceof FSVariableBox) {
			...
			NodeAddFindingDialogListener nodeAddFindingDialogListener = 
				new NodeAddFindingDialogListener(this, editorPanel);
