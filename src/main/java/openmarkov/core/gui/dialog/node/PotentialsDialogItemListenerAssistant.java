...
public class PotentialsDialogItemListenerAssistant implements
	 ...
	 */
	private PotentialsDialog potentialsDialog = null;
	 ...
	 */
	public PotentialsDialogItemListenerAssistant(
								PotentialsDialog potentialsDialog) {
	 ...
	 * @return the panel
	 */
	public PotentialsDialog getPotentialsDialog() {

		return potentialsDialog;
	}
	 ...
	 *            the panel to set
	 */
	public void setPotentialsDialog(PotentialsDialog potentialsDialog) {

		this.potentialsDialog = potentialsDialog;
	}
	...
	public void itemStateChanged(ItemEvent e) {
		...
		if (e.getItem().equals(getPotentialsDialog().getJRadioButtonDeterministicType())) {
			itemStateChangedDeterministicType(e.getStateChange());
		}
		if (e.getItem().equals(getPotentialsDialog().getJRadioButtonOptimalType())) {
	...
	private void itemStateChangedOptimal(int itemEvent) {
		...
		int result;
		if (itemEvent == ItemEvent.SELECTED) {
			if (getPotentialsDialog().getPreviousPolicy() == -1) {
	...
	private void itemStateChangedProbabilisticType(int itemEvent) {
		...
		int result;
		if (itemEvent == ItemEvent.SELECTED) {
			if (getPotentialsDialog().getPreviousPolicy() == -1) { // no
				// change.
				// First
				// time.
				result = JOptionPane.YES_OPTION;
	...
	private void itemStateChangedNeto(int itemEvent) {
		...
		if (itemEvent == ItemEvent.SELECTED) {
			getPotentialsDialog().setShowNetValues(true);
		}
	}
	...
	private void itemStateChangedCompound(int itemEvent) {
		...
		if (itemEvent == ItemEvent.SELECTED) {
			getPotentialsDialog().setShowNetValues(false);
