package openmarkov.core.gui.dialog.comment.hexidec.ekit;
...
import openmarkov.core.gui.dialog.comment.hexidec.ekit.action.CustomAction;
import openmarkov.core.gui.dialog.comment.hexidec.ekit.action.FormatAction;
import openmarkov.core.gui.dialog.comment.hexidec.ekit.action.ListAutomationAction;
import openmarkov.core.gui.dialog.comment.hexidec.ekit.action.SetFontFamilyAction;
import openmarkov.core.gui.dialog.comment.hexidec.ekit.action.StylesAction;
import openmarkov.core.gui.dialog.comment.hexidec.ekit.component.ExtendedHTMLDocument;
import openmarkov.core.gui.dialog.comment.hexidec.ekit.component.ExtendedHTMLEditorKit;
import openmarkov.core.gui.dialog.comment.hexidec.ekit.component.HTMLUtilities;
import openmarkov.core.gui.dialog.comment.hexidec.ekit.component.ImageFileChooser;
import openmarkov.core.gui.dialog.comment.hexidec.ekit.component.ImageURLDialog;
import openmarkov.core.gui.dialog.comment.hexidec.ekit.component.JButtonNoFocus;
import openmarkov.core.gui.dialog.comment.hexidec.ekit.component.JComboBoxNoFocus;
import openmarkov.core.gui.dialog.comment.hexidec.ekit.component.JToggleButtonNoFocus;
import openmarkov.core.gui.dialog.comment.hexidec.ekit.component.MutableFilter;
import openmarkov.core.gui.dialog.comment.hexidec.ekit.component.PropertiesDialog;
import openmarkov.core.gui.dialog.comment.hexidec.ekit.component.SearchDialog;
import openmarkov.core.gui.dialog.comment.hexidec.ekit.component.SimpleInfoDialog;
import openmarkov.core.gui.dialog.comment.hexidec.ekit.component.UnicodeDialog;
...
public class EkitCore extends JPanel implements ActionListener, KeyListener,
	...
	public static final String TOOLBAR_DEFAULT_MULTI =
		"NW|OP|SV|PR|SP|CT|CP|PS|SP|UN|RE|SP|FN|SP|UC|UM|SP|SR|*|BL|IT|UD|SP|SK|SU|SB|SP|AL|AC|AR|AJ|SP|UL|OL|SP|LK|*|ST|SP|FO";
	...
	public EkitCore(String sDocument, String sStyleSheet, String sRawDocument,
					...
					boolean debugMode, boolean hasSpellChecker,
		...
		actionFontStrike =
			new FormatAction(this, Translatrix
				.getTranslationString("FontStrike"), HTML.Tag.STRIKE);
		actionFontSuperscript =
			new FormatAction(this, Translatrix
				.getTranslationString("FontSuperscript"), HTML.Tag.SUP);
		actionFontSubscript =
			new FormatAction(this, Translatrix
				.getTranslationString("FontSubscript"), HTML.Tag.SUB);
		actionListUnordered =
		...
		actionAlignLeft =
			new StyledEditorKit.AlignmentAction(Translatrix
		...
		actionInsertAnchor =
			new CustomAction(this, Translatrix
				.getTranslationString("InsertAnchor")
				+ menuDialog, HTML.Tag.A);
		...
		JMenuItem jmiSaveAs =
			new JMenuItem(Translatrix.getTranslationString("SaveAs")
				+ menuDialog);
		jmiSaveAs.setActionCommand("saveas");
		jmiSaveAs.addActionListener(this);
		jMenuFile.add(jmiSaveAs);
		JMenuItem jmiSaveBody =
			new JMenuItem(Translatrix.getTranslationString("SaveBody")
				+ menuDialog);
		jmiSaveBody.setActionCommand("savebody");
		jmiSaveBody.addActionListener(this);
		jMenuFile.add(jmiSaveBody);
		JMenuItem jmiSaveRTF =
			new JMenuItem(Translatrix.getTranslationString("SaveRTF")
				+ menuDialog);
		jmiSaveRTF.setActionCommand("savertf");
		jmiSaveRTF.addActionListener(this);
		jMenuFile.add(jmiSaveRTF);
		JMenuItem jmiSaveB64 =
			new JMenuItem(Translatrix.getTranslationString("SaveB64")
				+ menuDialog);
		...
		jMenuFile.addSeparator();
		JMenuItem jmiPrint =
			new JMenuItem(Translatrix.getTranslationString("Print"));
		jmiPrint.setActionCommand("print");
		jmiPrint.addActionListener(this);
		jMenuFile.add(jmiPrint);
		jMenuFile.addSeparator();
		JMenuItem jmiSerialOut =
			new JMenuItem(Translatrix.getTranslationString("Serialize")
				+ menuDialog);
		jmiSerialOut.setActionCommand("serialize");
		jmiSerialOut.addActionListener(this);
		jMenuFile.add(jmiSerialOut);
		JMenuItem jmiSerialIn =
			new JMenuItem(Translatrix.getTranslationString("ReadFromSer")
				+ menuDialog);
		...
		jMenuFile.addSeparator();
		JMenuItem jmiExit =
			new JMenuItem(Translatrix.getTranslationString("Exit"));
		...
		if (includeToolBar) {
			if (multiBar) {
				jMenuToolbars =
					new JMenu(Translatrix.getTranslationString("ViewToolbars"));
		...
		jmiBold.setAccelerator(KeyStroke.getKeyStroke(
			'B', KeyEvent.CTRL_MASK, false));
		if (showMenuIcons) {
		...
		jmiItalic.setAccelerator(KeyStroke.getKeyStroke(
			'I', KeyEvent.CTRL_MASK, false));
		if (showMenuIcons) {
		...
		jmiUnderline.setAccelerator(KeyStroke.getKeyStroke(
			'U', KeyEvent.CTRL_MASK, false));
		if (showMenuIcons) {
		...
		JMenu jMenuFontSize =
			new JMenu(Translatrix.getTranslationString("FontSize"));
		...
		jMenuFont.addSeparator();
		JMenu jMenuFontSub =
			new JMenu(Translatrix.getTranslationString("Font"));
		...
		JMenuItem jmiSerif =
			new JMenuItem((Action) actions.get("font-family-Serif"));
		jmiSerif.setText(Translatrix.getTranslationString("FontSerif"));
		jMenuFontSub.add(jmiSerif);
		JMenuItem jmiSansSerif =
			new JMenuItem((Action) actions.get("font-family-SansSerif"));
		jmiSansSerif.setText(Translatrix.getTranslationString("FontSansserif"));
		jMenuFontSub.add(jmiSansSerif);
		JMenuItem jmiMonospaced =
			new JMenuItem((Action) actions.get("font-family-Monospaced"));
		...
		JMenu jMenuFontColor =
			new JMenu(Translatrix.getTranslationString("Color"));
		...
		jMenuFontColor.add(new JMenuItem(new CustomAction(this, Translatrix
			.getTranslationString("CustomColor")
			+ menuDialog, HTML.Tag.FONT, customAttr)));
		jMenuFontColor.add(new JMenuItem(new StyledEditorKit.ForegroundAction(
			Translatrix.getTranslationString("ColorAqua"), new Color(0, 255,
				255))));
		jMenuFontColor
			.add(new JMenuItem(new StyledEditorKit.ForegroundAction(Translatrix
				.getTranslationString("ColorBlack"), new Color(0, 0, 0))));
		jMenuFontColor.add(new JMenuItem(
			new StyledEditorKit.ForegroundAction(Translatrix
				.getTranslationString("ColorBlue"), new Color(0, 0, 255))));
		jMenuFontColor.add(new JMenuItem(new StyledEditorKit.ForegroundAction(
			Translatrix.getTranslationString("ColorFuschia"), new Color(255, 0,
				255))));
		jMenuFontColor.add(new JMenuItem(new StyledEditorKit.ForegroundAction(
			Translatrix.getTranslationString("ColorGray"), new Color(128, 128,
				128))));
		jMenuFontColor.add(new JMenuItem(new StyledEditorKit.ForegroundAction(
			Translatrix.getTranslationString("ColorGreen"),
			new Color(0, 128, 0))));
		jMenuFontColor.add(new JMenuItem(
			new StyledEditorKit.ForegroundAction(Translatrix
				.getTranslationString("ColorLime"), new Color(0, 255, 0))));
		jMenuFontColor.add(new JMenuItem(new StyledEditorKit.ForegroundAction(
			Translatrix.getTranslationString("ColorMaroon"), new Color(128, 0,
				0))));
		jMenuFontColor.add(new JMenuItem(
			new StyledEditorKit.ForegroundAction(Translatrix
				.getTranslationString("ColorNavy"), new Color(0, 0, 128))));
		jMenuFontColor.add(new JMenuItem(new StyledEditorKit.ForegroundAction(
			Translatrix.getTranslationString("ColorOlive"), new Color(128, 128,
				0))));
		jMenuFontColor.add(new JMenuItem(new StyledEditorKit.ForegroundAction(
			Translatrix.getTranslationString("ColorPurple"), new Color(128, 0,
				128))));
		jMenuFontColor
			.add(new JMenuItem(new StyledEditorKit.ForegroundAction(Translatrix
		...
		jMenuFontColor.add(new JMenuItem(new StyledEditorKit.ForegroundAction(
			Translatrix.getTranslationString("ColorSilver"), new Color(192,
				192, 192))));
		jMenuFontColor.add(new JMenuItem(new StyledEditorKit.ForegroundAction(
			Translatrix.getTranslationString("ColorTeal"), new Color(0, 128,
				128))));
		jMenuFontColor.add(new JMenuItem(new StyledEditorKit.ForegroundAction(
			Translatrix.getTranslationString("ColorWhite"), new Color(255, 255,
				255))));
		jMenuFontColor.add(new JMenuItem(new StyledEditorKit.ForegroundAction(
		...
		JMenu jMenuFormatAlign =
			new JMenu(Translatrix.getTranslationString("Align"));
		...
		JMenu jMenuFormatHeading =
			new JMenu(Translatrix.getTranslationString("Heading"));
		...
		JMenuItem jmiBreak =
			new JMenuItem(Translatrix.getTranslationString("InsertBreak"));
		jmiBreak.setActionCommand("insertbreak");
		jmiBreak.addActionListener(this);
		jmiBreak.setAccelerator(KeyStroke.getKeyStroke(
			KeyEvent.VK_ENTER, KeyEvent.SHIFT_MASK, false));
		jMenuInsert.add(jmiBreak);
		JMenuItem jmiNBSP =
			new JMenuItem(Translatrix.getTranslationString("InsertNBSP"));
		jmiNBSP.setActionCommand("insertnbsp");
		jmiNBSP.addActionListener(this);
		jmiNBSP.setAccelerator(KeyStroke.getKeyStroke(
			KeyEvent.VK_SPACE, KeyEvent.SHIFT_MASK, false));
		jMenuInsert.add(jmiNBSP);
		JMenu jMenuUnicode =
			new JMenu(Translatrix
				.getTranslationString("InsertUnicodeCharacter"));
		if (showMenuIcons) {
			jMenuUnicode.setIcon(getEkitIcon("Unicode"));
		}
		;
		JMenuItem jmiUnicodeAll =
			new JMenuItem(Translatrix
				.getTranslationString("InsertUnicodeCharacterAll")
				+ menuDialog);
		if (showMenuIcons) {
		...
		JMenuItem jmiUnicodeMath =
			new JMenuItem(Translatrix
				.getTranslationString("InsertUnicodeCharacterMath")
				+ menuDialog);
		if (showMenuIcons) {
		...
		JMenuItem jmiUnicodeDraw =
			new JMenuItem(Translatrix
				.getTranslationString("InsertUnicodeCharacterDraw")
				+ menuDialog);
		if (showMenuIcons) {
		...
		jmiHRule.setText(Translatrix
			.getTranslationString("InsertHorizontalRule"));
		jMenuInsert.add(jmiHRule);
		jMenuInsert.addSeparator();
		JMenuItem jmiImageLocal =
			new JMenuItem(Translatrix.getTranslationString("InsertLocalImage")
				+ menuDialog);
		jmiImageLocal.setActionCommand("insertlocalimage");
		jmiImageLocal.addActionListener(this);
		jMenuInsert.add(jmiImageLocal);
		JMenuItem jmiImageURL =
			new JMenuItem(Translatrix.getTranslationString("InsertURLImage")
				+ menuDialog);
		jmiImageURL.setActionCommand("inserturlimage");
		...
		JMenuItem jmiTable =
			new JMenuItem(Translatrix.getTranslationString("InsertTable")
				+ menuDialog);
		if (showMenuIcons) {
		...
		JMenuItem jmiEditTable =
			new JMenuItem(Translatrix.getTranslationString("TableEdit")
				+ menuDialog);
		if (showMenuIcons) {
		...
		JMenuItem jmiEditCell =
			new JMenuItem(Translatrix.getTranslationString("TableCellEdit")
				+ menuDialog);
		if (showMenuIcons) {
		...
		jMenuTable.addSeparator();
		JMenuItem jmiTableRow =
			new JMenuItem(Translatrix.getTranslationString("InsertTableRow"));
		if (showMenuIcons) {
		...
		JMenuItem jmiTableCol =
			new JMenuItem(Translatrix.getTranslationString("InsertTableColumn"));
		if (showMenuIcons) {
		...
		jMenuTable.addSeparator();
		JMenuItem jmiTableRowDel =
			new JMenuItem(Translatrix.getTranslationString("DeleteTableRow"));
		if (showMenuIcons) {
		...
		JMenuItem jmiTableColDel =
			new JMenuItem(Translatrix.getTranslationString("DeleteTableColumn"));
		if (showMenuIcons) {
		...
		JMenuItem jmiFormInsertForm =
			new JMenuItem(Translatrix.getTranslationString("FormInsertForm"));
		...
		JMenuItem jmiFormButtonReset =
			new JMenuItem(Translatrix.getTranslationString("FormButtonReset"));
		...
		if (hasSpellChecker) {
			...
			JMenuItem jmiSpellcheck =
				new JMenuItem(Translatrix
					.getTranslationString("ToolSpellcheck"));
		...
		JMenuItem jmiFind =
			new JMenuItem(Translatrix.getTranslationString("SearchFind"));
		if (showMenuIcons) {
		...
		jmiFind.setAccelerator(KeyStroke.getKeyStroke(
			'F', KeyEvent.CTRL_MASK, false));
		jMenuSearch.add(jmiFind);
		JMenuItem jmiFindAgain =
			new JMenuItem(Translatrix.getTranslationString("SearchFindAgain"));
		if (showMenuIcons) {
		...
		jmiFindAgain.setAccelerator(KeyStroke.getKeyStroke(
			'G', KeyEvent.CTRL_MASK, false));
		jMenuSearch.add(jmiFindAgain);
		JMenuItem jmiReplace =
			new JMenuItem(Translatrix.getTranslationString("SearchReplace"));
		if (showMenuIcons) {
		...
		jmiReplace.setAccelerator(KeyStroke.getKeyStroke(
			'R', KeyEvent.CTRL_MASK, false));
		...
		JMenuItem jmiAbout =
			new JMenuItem(Translatrix.getTranslationString("About"));
		...
		JMenuItem jmiDesc =
			new JMenuItem(Translatrix.getTranslationString("DescribeDoc"));
		jmiDesc.setActionCommand("describe");
		jmiDesc.addActionListener(this);
		jMenuDebug.add(jmiDesc);
		JMenuItem jmiDescCSS =
			new JMenuItem(Translatrix.getTranslationString("DescribeCSS"));
		jmiDescCSS.setActionCommand("describecss");
		jmiDescCSS.addActionListener(this);
		jMenuDebug.add(jmiDescCSS);
		JMenuItem jmiTag =
			new JMenuItem(Translatrix.getTranslationString("WhatTags"));
		...
		if (multiBar) {
			...
			initialisesingleToolbar(toolbarSeq);
	...
	public EkitCore(String sRawDocument, URL urlStyleSheet,
		...
		this(null, null, sRawDocument, (StyledDocument) null, urlStyleSheet,
			includeToolBar, showViewSource, showMenuIcons, editModeExclusive,
			sLanguage, sCountry, base64, false, hasSpellChecker, multiBar,
			toolbarSeq);
	}
	...
	public EkitCore() {
		...
		this((String) null, (String) null, (String) null,
			(StyledDocument) null, (URL) null, true, false, true, true,
			(String) null, (String) null, false, false, false, true,
			TOOLBAR_DEFAULT_MULTI);
	}
		...
		try {
			...
			if (command.equals("newdoc")) {
				SimpleInfoDialog sidAsk =
					new SimpleInfoDialog(this.getFrame(), "", true, Translatrix
						.getTranslationString("AskNewDocument"),
				...
				String[] fieldNames =
					{ "rows", "cols", "border", "cellspacing", "cellpadding",
						"width", "valign" };
				String[] fieldTypes =
					{ "text", "text", "text", "text", "text", "text", "combo" };
				String[] fieldValues =
					{ "3", "3", "1", "2", "4", "100%", "top,middle,bottom" };
				insertTable(
					(Hashtable) null, fieldNames, fieldTypes, fieldValues);
				...
				insertFormElement(
					HTML.Tag.FORM, "form", (Hashtable) null, fieldNames,
					fieldTypes, fieldValues, true);
				...
				insertFormElement(
					HTML.Tag.INPUT, "input", htAttribs, fieldNames, fieldTypes,
					false);
			} else if (command.equals("inserttextarea")) {
				String[] fieldNames = { "name", "rows", "cols" };
				String[] fieldTypes = { "text", "text", "text" };
				insertFormElement(
					HTML.Tag.TEXTAREA, "textarea", (Hashtable) null,
					fieldNames, fieldTypes, true);
				...
				doSearch(
					(String) null, (String) null, false, lastSearchCaseSetting,
					lastSearchTopSetting);
			} else if (command.equals("findagain")) {
				doSearch(
					lastSearchFindTerm, (String) null, false,
					lastSearchCaseSetting, false);
			} else if (command.equals("replace")) {
				doSearch(
					(String) null, (String) null, true, lastSearchCaseSetting,
					lastSearchTopSetting);
			...
			SimpleInfoDialog sidAbout =
				new SimpleInfoDialog(this.getFrame(), Translatrix
					...
					.getTranslationString("ErrorIOException"),
			...
			logException(
				"ClassNotFound Exception in actionPerformed method", cnfe);
			SimpleInfoDialog sidAbout =
				new SimpleInfoDialog(this.getFrame(), Translatrix
					.getTranslationString("Error"), true, Translatrix
			...
			SimpleInfoDialog sidAbout =
				new SimpleInfoDialog(this.getFrame(), Translatrix
					.getTranslationString("Error"), true, Translatrix
					.getTranslationString("ErrorRuntimeException"),
					SimpleInfoDialog.ERROR);
		}
	}
		...
		if (ke.getKeyChar() == KeyEvent.VK_BACK_SPACE) {
			try {
				if (pos > 0) {
					if ((selectedText = jtpMain.getSelectedText()) != null) {
						...
						int sOffset =
							htmlDoc.getParagraphElement(pos).getStartOffset();
			...
			try {
				if (htmlUtilities.checkParentsTag(HTML.Tag.UL) == true
					...
					int eo = elem.getEndOffset();
					char[] temp =
						this.getTextPane().getText(so, eo - so).toCharArray();
					...
					if (elem.getStartOffset() == elem.getEndOffset()
						...
						if (this.getCaretPosition() + 1 == elem.getEndOffset()) {
							...
							String tempString =
								this.getTextPane().getText(caret, eo - caret);
	...
	private void insertTable(Hashtable attribs, String[] fieldNames,
								String[] fieldTypes, String[] fieldValues)
		...
		if (fieldNames != null && fieldNames.length > 0) {
			PropertiesDialog propertiesDialog =
				new PropertiesDialog(this.getFrame(), fieldNames, fieldTypes,
					fieldValues, Translatrix
			...
			if (decision.equals(Translatrix
				...
				for (int iter = 0; iter < fieldNames.length; iter++) {
					...
					String propValue =
						propertiesDialog.getFieldValue(fieldName);
		...
		htmlKit.insertHTML(
			htmlDoc, caretPos, compositeElement.toString(), 0, 0,
			HTML.Tag.TABLE);
		...
		if (elementParent != null) {
			HTML.Attribute[] fieldKeys =
				{ HTML.Attribute.BORDER, HTML.Attribute.CELLSPACING,
					HTML.Attribute.CELLPADDING, HTML.Attribute.WIDTH,
					HTML.Attribute.VALIGN };
			String[] fieldNames =
				{ "border", "cellspacing", "cellpadding", "width", "valign" };
			String[] fieldTypes = { "text", "text", "text", "text", "combo" };
			String[] fieldValues = { "", "", "", "", "top,middle,bottom," };
			MutableAttributeSet myatr =
				(MutableAttributeSet) elementParent.getAttributes();
			for (int i = 0; i < fieldNames.length; i++) {
				if (myatr.isDefined(fieldKeys[i])) {
					if (fieldTypes[i].equals("combo")) {
						fieldValues[i] =
							myatr.getAttribute(fieldKeys[i]).toString() + ","
								+ fieldValues[i];
					} else {
						fieldValues[i] =
							myatr.getAttribute(fieldKeys[i]).toString();
			...
			PropertiesDialog propertiesDialog =
				new PropertiesDialog(this.getFrame(), fieldNames, fieldTypes,
					fieldValues, Translatrix.getTranslationString("TableEdit"),
					true);
			propertiesDialog.setVisible(true);
			if (!propertiesDialog.getDecisionValue().equals(
				...
				for (int i = 0; i < fieldNames.length; i++) {
					String propValue =
						propertiesDialog.getFieldValue(fieldNames[i]);
					if (propValue != null && propValue.length() > 0) {
						myAtributes =
							myAtributes + fieldNames[i] + "=\"" + propValue
								+ "\" ";
			...
			SimpleInfoDialog sidAbout =
				new SimpleInfoDialog(this.getFrame(), Translatrix
					.getTranslationString("Table"), true, Translatrix
	...
	private void editCell() {
		...
		if (elementParent != null) {
			HTML.Attribute[] fieldKeys =
				{ HTML.Attribute.WIDTH, HTML.Attribute.HEIGHT,
					HTML.Attribute.ALIGN, HTML.Attribute.VALIGN,
			...
			SimpleInfoDialog sidAbout =
				new SimpleInfoDialog(this.getFrame(), Translatrix
					.getTranslationString("Cell"), true, Translatrix
	...
	private void insertTableRow() {
		...
		int caretPos = jtpMain.getCaretPosition();
		Element element =
			htmlDoc.getCharacterElement(jtpMain.getCaretPosition());
		Element elementParent = element.getParentElement();
		int startPoint = -1;
		int columnCount = -1;
		...
		if (startPoint > -1 && columnCount > -1) {
			...
			ActionEvent actionEvent =
				new ActionEvent(jtpMain, 0, "insertTableRow");
	...
	private void insertTableColumn() {
		...
		if (startPoint > -1 && rowCount > -1) {
			...
			String sCell = "<TD></TD>";
			ActionEvent actionEvent =
				new ActionEvent(jtpMain, 0, "insertTableCell");
			for (int i = 0; i < rowCount; i++) {
				...
				new HTMLEditorKit.InsertHTMLTextAction("insertTableCell",
					sCell, HTML.Tag.TR, HTML.Tag.TD, HTML.Tag.TH, HTML.Tag.TD)
					.actionPerformed(actionEvent);
			}
			refreshOnUpdate();
			jtpMain.setCaretPosition(caretPos);
		...
		if (elementCell != null && elementRow != null && elementTable != null) {
			...
			if (whichColumn > -1) {
				...
				for (int i = 0; i < elementTable.getElementCount(); i++) {
					...
					elementCell =
						(elementRow.getElementCount() > whichColumn
							? elementRow.getElement(whichColumn) : elementRow
	...
	private void insertUnicode(int index) throws IOException,
		...
		UnicodeDialog unicodeInput =
			new UnicodeDialog(this, Translatrix
				.getTranslationString("UnicodeDialogTitle"), false, index);
	}
	...
	private void insertFormElement(HTML.Tag baseTag, String baseElement,
									Hashtable attribs, String[] fieldNames,
									String[] fieldTypes, String[] fieldValues,
									boolean hasClosingTag) throws IOException,
					BadLocationException, RuntimeException {
		...
		htmlKit.insertHTML(
			htmlDoc, caretPos, compositeElement.toString(), 0, 0, baseTag);
		// jtpMain.setCaretPosition(caretPos + 1);
		refreshOnUpdate();
	}
	...
	private void insertFormElement(HTML.Tag baseTag, String baseElement,
									Hashtable attribs, String[] fieldNames,
									String[] fieldTypes, boolean hasClosingTag)
					throws IOException, BadLocationException, RuntimeException {

		insertFormElement(
			baseTag, baseElement, attribs, fieldNames, fieldTypes,
			new String[fieldNames.length], hasClosingTag);
	}
	...
	private void doSearch(String searchFindTerm, String searchReplaceTerm,
		...
		if (searchFindTerm != null
			...
			if (bReplaceAll) {
				int results =
					findText(
						searchFindTerm, searchReplaceTerm, bCaseSensitive, 0);
				int findOffset = 0;
				if (results > -1) {
					while (results > -1) {
				...
				int results =
					findText(
						searchFindTerm, searchReplaceTerm, bCaseSensitive,
	...
	private int findText(String findTerm, String replaceTerm,
		...
		try {
			...
			searchPlace =
				(bCaseSenstive ? baseDocument.getText(
					0, baseDocument.getLength()).indexOf(findTerm, iOffset)
	...
	private void insertLocalImage(File whatImage) throws IOException,
		...
		if (whatImage == null) {
			whatImage =
				getImageFromChooser(imageChooserStartDir, extsIMG, Translatrix
	...
	public void insertURLImage() throws IOException, BadLocationException,
		...
		ImageURLDialog iurlDialog =
			new ImageURLDialog(this.getFrame(), Translatrix
	...
	private void writeOut(HTMLDocument doc, File whatFile) throws IOException,
		...
		if (whatFile == null) {
			whatFile =
				getFileFromChooser(
					".", JFileChooser.SAVE_DIALOG, extsHTML, Translatrix
	...
	private void writeOutFragment(HTMLDocument doc, String containingTag)
		...
		if (whatFile != null) {
			...
			int tagStart =
				docTextCase.indexOf("<" + containingTag.toLowerCase());
			...
			String bodyText =
				jtpSource.getText().substring(tagStartClose, tagEndOpen);
	...
	private void writeOutRTF(StyledDocument doc) throws IOException,
		...
		File whatFile =
			getFileFromChooser(
				".", JFileChooser.SAVE_DIALOG, extsRTF, Translatrix
	...
	private void writeOutBase64(String text) throws IOException,
		...
		File whatFile =
			getFileFromChooser(
				".", JFileChooser.SAVE_DIALOG, extsB64, Translatrix
	...
	private void openDocument(File whatFile) throws IOException,
		...
		if (whatFile == null) {
			whatFile =
				getFileFromChooser(
					".", JFileChooser.OPEN_DIALOG, extsHTML, Translatrix
						.getTranslationString("FiletypeHTML"));
		}
		if (whatFile != null) {
			try {
	...
	private void openDocumentBase64(File whatFile) throws IOException,
		...
		if (whatFile == null) {
			whatFile =
				getFileFromChooser(
					".", JFileChooser.OPEN_DIALOG, extsB64, Translatrix
						.getTranslationString("FiletypeB64"));
		}
		if (whatFile != null) {
	...
	private void openStyleSheet(File fileCSS) throws IOException {
		...
		if (fileCSS == null) {
			fileCSS =
				getFileFromChooser(
					".", JFileChooser.OPEN_DIALOG, extsCSS, Translatrix
	...
	public void serializeOut(HTMLDocument doc) throws IOException {
		...
		File whatFile =
			getFileFromChooser(
				".", JFileChooser.SAVE_DIALOG, extsSer, Translatrix
	...
	public void serializeIn() throws IOException, ClassNotFoundException {
		...
		File whatFile =
			getFileFromChooser(
				".", JFileChooser.OPEN_DIALOG, extsSer, Translatrix
					.getTranslationString("FiletypeSer"));
		if (whatFile != null) {
	...
	private File getImageFromChooser(String startDir, String[] exts, String desc) {
		...
		int optionSelected = JFileChooser.CANCEL_OPTION;
		optionSelected =
			jImageDialog.showDialog(this, Translatrix
	...
	private Element locateElementInDocument(StyledDocument doc,
		...
		for (int i = 0; i < elements.length; i++) {
			if (elements[i].getName().equalsIgnoreCase(elementName)) {
				...
				Element rtnElement =
					locateChildElementInDocument(elements[i], elementName);
				if (rtnElement != null) {
					return rtnElement;
				}
			}
	...
	public void initializeMultiToolbars(String toolbarSeq) {
		...
		int whichBar = 0;
		StringTokenizer stToolbars =
			new StringTokenizer(toolbarSeq.toUpperCase(), "|");
	 ...
	 */
	public void initialisesingleToolbar(String toolbarSeq) {
	...
	public String getFontNameFromSelector() {
		...
		if (jcmbFontSelector == null
			|| jcmbFontSelector.getSelectedItem().equals(
				Translatrix
					.getTranslationString("SelectorToolFontsDefaultFont"))) {
			return (String) null;
		} else {
