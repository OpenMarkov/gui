...
/**
 * SAXDataLoader implements a Data Loader from XML external file to help in the
 * Menu definition. It reads from the file the menu structure, handlers and
 * Toolbars In such way, it is not required to implement specific classes for
 * Menus, Toolbars and HandlerAction in the application.
 * 
 * @author jlgozalo
 * @version 1.0
 */

public class SaxDataLoader extends DefaultHandler implements ActionListener,
				ItemListener, SAXDataLoaderTokens {
	...
	private static SaxDataLoader saxDataLoader = null;

	// Constructor
	private SaxDataLoader() {
	...
	/** @return SAXDataLoader unique instance (singleton pattern). */
	public static SaxDataLoader getUniqueInstance() {

		if (saxDataLoader == null) {
			saxDataLoader = new SaxDataLoader();
		}
		return saxDataLoader;
	}
	 ...
	 * @throws LoadingMenusException
	 *             when an error ocurrs
	 */
	public void loadData(URI uri) throws LoadingMenusException {
		...
		try {
			...
			throw new LoadingMenusException("Imposible to start without menus");
		} catch (Exception ex) {
			System.err.println("SaxDataFactory.loadData 1: "
	 ...
	 *             when an error ocurrs
	 */
	public void loadData(File file) throws LoadingMenusException {
		...
		try {
			...
			System.err.println("SaxDataFactory.loadData 2: " + file + ": "
				+ ex.getMessage());
			throw new LoadingMenusException("Imposible to start without menus");
		}
	}
	 ...
	 *             when an error ocurrs
	 */
	public void loadData(String sFilename) throws LoadingMenusException {
	 ...
	 *             when an error ocurrs
	 */
	public void loadData(InputStream istream) throws LoadingMenusException {
		...
		try {
			...
			System.err.println("SaxDataFactory.loadData 3: " + istream + ": "
				+ ex.getMessage());
			throw new LoadingMenusException("Imposible to start without menus");
		}
	}
	 ...
	 * @return menuBar corresponding to the Menu name
	 */
	protected JMenuBar menubarFind() {

		Object oToReturn = htMenuBars.elements();
		if (oToReturn != null && oToReturn instanceof JMenuBar) {
			return (JMenuBar) oToReturn;
		}
		return null;
