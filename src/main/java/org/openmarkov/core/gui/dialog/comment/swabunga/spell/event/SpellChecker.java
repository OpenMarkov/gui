package org.openmarkov.core.gui.dialog.comment.swabunga.spell.event;
...
import org.openmarkov.core.gui.dialog.comment.swabunga.spell.engine.Configuration;
import org.openmarkov.core.gui.dialog.comment.swabunga.spell.engine.SpellDictionary;
import org.openmarkov.core.gui.dialog.comment.swabunga.spell.engine.Word;
...
public class SpellChecker {
	 ...
	 * @param word
	 *            Description of the Parameter
	 * @param startsSentance
	 *            True if this word is at the start of a sentance
	 * @return The mixedCaseWord value
	 */
	private final static boolean isMixedCaseWord(String word,
													boolean startsSentance) {

		int strLen = word.length();
		boolean isUpper = Character.isUpperCase(word.charAt(0));
		// Ignore the first character if this word starts the sentance and the
		// first
	...
	public final int checkSpelling(WordTokenizer tokenizer) {
		...
		while (tokenizer.hasMoreWords() && !terminated) {
			...
			if (!dictionary.isCorrect(word)) {
				...
				if ((!config
					.getBoolean(Configuration.SPELL_IGNORESENTANCECAPITALIZATION))
					&& (tokenizer.isNewSentance())
					...
					SpellCheckEvent event =
						new BasicSpellCheckEvent(word, suggestion, tokenizer);
					terminated = fireAndHandleEvent(tokenizer, event);
				}
			}
		}
