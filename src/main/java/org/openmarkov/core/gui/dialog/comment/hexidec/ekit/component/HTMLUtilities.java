/*
 * GNU Lesser General Public License HTMLUtilities - Special Utility Functions
 * For Ekit Copyright (C) 2003 Rafael Cieplinski & Howard Kistler This library
...
public class HTMLUtilities {
	...
	public HTMLUtilities(EkitCore newParent) {
		...
		/*
		 * HTML.Tag classTag = new HTML.Tag(); Field[] fields =
		 * classTag.getClass().getDeclaredFields(); for(int i = 0; i <
		 * fields.length; i++) { try { System.out.println("Adding " +
		 * (String)fields[i].get(null).toString() + " => " +
		 * (HTML.Tag)fields[i].get(null));
		 * tags.put((String)fields[i].get(null).toString(),
		 * (HTML.Tag)fields[i].get(null)); } catch (IllegalAccessException iae) { } }
		 */
	}
	...
	public void insertListElement(String content) {
		...
		newHtmlString.append("</li>");
		newHtmlString.append(source
			.substring(positions[3] + 1, source.length()));
	...
	public void removeTag(Element element, boolean closingTag) {
		...
		if (closingTag) {
			...
			newHtmlString.append(source.substring(0, beginStartTag));
			newHtmlString
				.append(source.substring(endStartTag, source.length()));
		}
		parent.getTextPane().setText(newHtmlString.toString());
		parent.refreshOnUpdate();
	}
	...
	private int[] getPositions(Element element, String source,
		...
		if (closingTag) {
			...
			// Schleife l�uft solange, bis keine neuen StartTags mehr gefunden
			// werden
			int interncaret = position[1];
			do {
				int temphitpoint = -1;
				boolean flaghitup = false;
				// Schleife sucht zwischen dem Start- und End-Tag nach neuen
				// Start-Tags
				hitUp = 0;
				do {
					flaghitup = false;
	...
	/*
	 * Diese Methode pr�ft ob der �bergebene Tag sich in der Hierachie nach oben
	 * befindet
	 */
	public boolean checkParentsTag(HTML.Tag tag) {

		Element e =
			parent.getExtendedHtmlDoc().getParagraphElement(
				parent.getCaretPosition());
	...
	/*
	 * Diese Methoden geben das erste gefundende dem �bergebenen tags
	 * entsprechende Element zur�ck
	 */
	public Element getListItemParent() {

		String listItemTag = HTML.Tag.LI.toString();
		Element eleSearch =
			parent.getExtendedHtmlDoc().getCharacterElement(
				parent.getCaretPosition());
		do {
	...
	/*
	 * Diese Methoden entfernen Attribute aus dem SimpleAttributeSet, gem�� den
	 * �bergebenen Werten, und geben das Ergebnis als SimpleAttributeSet zur�ck
	 */
	...
	public SimpleAttributeSet removeAttribute(SimpleAttributeSet sourceAS,
		...
		try {
			...
			while (sourceEn.hasMoreElements()) {
				...
				sourceValues[i] =
					(String) sourceAS.getAttribute(temp).toString();
				i++;
			}
			...
			while (removeEn.hasMoreElements()) {
				...
				removeValues[j] =
					(String) removeAS.getAttribute(removeKeys[j]).toString();
				j++;
			}
				...
				if (sourceKeys[countSource] == "name"
					...
					for (int countRemove = 0; countRemove < removeKeys.length; countRemove++) {
						if (removeKeys[countRemove] != "NULL") {
							if (sourceKeys[countSource].toString() == removeKeys[countRemove]
								.toString()) {
	...
	public void delete() throws BadLocationException, IOException {
		...
		htmlDoc.insertString(
			selEnd + posStrings[0].length(), posStrings[1], null);
		parent.refreshOnUpdate();
		...
		htmlString +=
			jtpSource.getText().substring(start + posStrings[0].length(), end);
		htmlString +=
			jtpSource.getText().substring(
				end + posStrings[1].length(), jtpSource.getText().length());
	...
	private String getAllTableTags(String source) throws BadLocationException,
					IOException {
		...
		do {
			...
			if (caret != -1) {
				result.append(source.substring(caret, source
					.indexOf(">", caret) + 1));
			}
		} while (caret != -1);
