/*
 * GNU Lesser General Public License Base64Codec - Base64 Document
 * Encoding/Decoding Class Copyright (C) 2003 Howard Kistler & other
 * contributors This library is free software; you can redistribute it and/or
...
package org.openmarkov.core.gui.dialog.comment.hexidec.util;
...
public class Base64Codec {
	...
	public static String encode(byte[] sourceBytes) {
		...
		while (bytePlace < sourceBytes.length) {
			tokenValue =
				(byte) ((byte) tokenValue | (byte) ((sourceBytes[bytePlace] & (1 << bitOffset)) > 0
					? (1 << b64Offset) : (byte) 0));
			bitOffset--;
			if (bitOffset < 0) {
				bitOffset = 7;
	...
	public static String decode(String source) {
		...
		while (bytePlace < source.length()) {
			...
			if (source.charAt(bytePlace) == Linefeed
				...
				if (!Base64Tokens.contains(source.substring(
					bytePlace, bytePlace + 1))) {
					...
					byte currentByte =
						(byte) (Base64Tokens.indexOf(source.substring(
							bytePlace, bytePlace + 1)));
					charValue =
						(byte) ((byte) charValue | (byte) ((currentByte & (1 << b64Offset)) > 0
							? (1 << bitOffset) : (byte) 0));
					bitOffset--;
					if (bitOffset < 0) {
						bitOffset = 7;
