...
package org.openmarkov.core.gui.dialog.comment.swabunga.spell.swing;
...
import org.openmarkov.core.gui.dialog.comment.swabunga.spell.event.SpellCheckEvent;
 ...
 * </p>
 * 
 * @author Jason Height (jheight@chariot.net.au) NOTE: Modification be Howard
 *         Kistler: Removed the calls that change the Invalid Word text box,
 *         because I think that behaviour is counter-intuitive and might even
 *         screw up other functions that rely on it being the original invalid
 *         word. If it's going to be like this for good, it should probably be a
 *         Label instead of a Textfield.
 */
public class JSpellForm extends JPanel implements ActionListener,
	 ...
	 * listener
	 */
	private static final JButton createButton(String command, String text,
	...
	protected JPanel makeEastPanel() {
		...
		JButton ignoreBtn =
			createButton(IGNORE_CMD, messages.getString(IGNORE_CMD), this);
		ignoreBtn
			.setMaximumSize(new Dimension(Short.MAX_VALUE, Short.MAX_VALUE));
		jPanel1.add(ignoreBtn);

		JButton ignoreAllBtn =
			createButton(IGNOREALL_CMD, messages.getString(IGNOREALL_CMD), this);
		ignoreAllBtn.setMaximumSize(new Dimension(Short.MAX_VALUE,
			Short.MAX_VALUE));
		jPanel1.add(ignoreAllBtn);

		JButton addBtn =
			createButton(ADD_CMD, messages.getString(ADD_CMD), this);
		addBtn.setMaximumSize(new Dimension(Short.MAX_VALUE, Short.MAX_VALUE));
		jPanel1.add(addBtn);

		JButton changeBtn =
			createButton(REPLACE_CMD, messages.getString(REPLACE_CMD), this);
		changeBtn
			.setMaximumSize(new Dimension(Short.MAX_VALUE, Short.MAX_VALUE));
		jPanel1.add(changeBtn);

		JButton changeAllBtn =
			createButton(
				REPLACEALL_CMD, messages.getString(REPLACEALL_CMD), this);
		changeAllBtn.setMaximumSize(new Dimension(Short.MAX_VALUE,
			Short.MAX_VALUE));
		jPanel1.add(changeAllBtn);

		JButton cancelBtn =
			createButton(CANCEL_CMD, messages.getString(CANCEL_CMD), this);
		cancelBtn
			.setMaximumSize(new Dimension(Short.MAX_VALUE, Short.MAX_VALUE));
