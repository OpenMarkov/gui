/*
 * GNU Lesser General Public License SimpleInfoDialog Copyright (C) 2000 Howard
...
public class SimpleInfoDialog extends JDialog {
	...
	public SimpleInfoDialog(Frame parent, String title, boolean bModal,
		...
		if (type == QUESTION) {
			...
			jOptionPane =
				new JOptionPane(new JLabel(message, SwingConstants.CENTER),
		...
		this.pack();
		int centerX =
			(int) (((parent.getSize().getWidth() / 2) + parent.getLocation()
				.getX()) - (this.getSize().getWidth() / 2));
		int centerY =
			(int) (((parent.getSize().getHeight() / 2) + parent.getLocation()
				.getY()) - (this.getSize().getHeight() / 2));
		if (centerX < 0) {
			centerX = 0;
		}
		if (centerY < 0) {
