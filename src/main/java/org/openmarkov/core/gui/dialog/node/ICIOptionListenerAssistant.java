...
/**
 * This class is an event manager to detect selected buttons in ICIOptionsPanel to change from ICIPotentialsTablePanel
 * ICIOptionsPanel starts with Canonical JRadiobutton selected (it is shown in ICIPotentialsTablePanel), then when it is selected CPT JRadiobutton
 * generates a CPTablePanel to get the JScrollPane that is responsible to display the complete parameters table values.
 * 
 * @author myebra
 *
 */
	...
	private void itemStateChangedTPC(ItemEvent e) {
		if (e.getStateChange() == ItemEvent.DESELECTED){
			...
			if ( previousModel == CANONICAL) { //Canonical --> tpc
				...
				ProbNode iciProbnode = new ProbNode(((ICIPotentialsTablePanel) parentPanel).getProbNode());
