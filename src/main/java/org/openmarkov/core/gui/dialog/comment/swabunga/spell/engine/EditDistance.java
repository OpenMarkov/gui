...
public class EditDistance {
	...
	public static final int getDistance(String word, String similar) {
		...
		for (int j = 1; j != b_size; ++j)
			matrix[0][j] =
				matrix[0][j - 1] + config.getInteger(Configuration.EDIT_DEL1);
		word = " " + word;
		similar = " " + similar;
		int te;
		for (int i = 1; i != a_size; ++i) {
			matrix[i][0] =
				matrix[i - 1][0] + config.getInteger(Configuration.EDIT_DEL2);
			for (int j = 1; j != b_size; ++j) {
				if (word.charAt(i) == similar.charAt(j)) {
					matrix[i][j] = matrix[i - 1][j - 1];
				} else {
					matrix[i][j] =
						config.getInteger(Configuration.EDIT_SUB)
							+ matrix[i - 1][j - 1];
					if (i != 1 && j != 1
						&& word.charAt(i) == similar.charAt(j - 1)
						&& word.charAt(i - 1) == similar.charAt(j)) {
						te =
							config.getInteger(Configuration.EDIT_SWAP)
								+ matrix[i - 2][j - 2];
						if (te < matrix[i][j])
							matrix[i][j] = te;
					}
					te =
						config.getInteger(Configuration.EDIT_DEL1)
							+ matrix[i - 1][j];
					if (te < matrix[i][j])
						matrix[i][j] = te;
					te =
						config.getInteger(Configuration.EDIT_DEL2)
							+ matrix[i][j - 1];
					if (te < matrix[i][j])
						matrix[i][j] = te;
				}
