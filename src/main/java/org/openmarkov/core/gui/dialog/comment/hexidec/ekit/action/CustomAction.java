/*
 * GNU Lesser General Public License CustomAction Copyright (C) 2000 Howard
...
package org.openmarkov.core.gui.dialog.comment.hexidec.ekit.action;
...
import org.openmarkov.core.gui.dialog.comment.hexidec.ekit.component.UserInputAnchorDialog;
...
public class CustomAction extends StyledEditorKit.StyledTextAction {
		...
		if (this.isEnabled()) {
			...
			if (selText == null || textLength < 1) {
				...
				if (htmlTag.toString().equals(HTML.Tag.A.toString())) {
					...
					for (int i = caretOffset; i < caretOffset
						+ internalTextLength; i++) {
						parentTextPane.select(i, i + 1);
						sasText =
							new SimpleAttributeSet(parentTextPane
								.getCharacterAttributes());
						Enumeration attribEntries1 =
							sasText.getAttributeNames();
						while (attribEntries1.hasMoreElements()
							...
							if (entryKey.toString().equals(
								...
								if (entryValue instanceof SimpleAttributeSet) {
									Enumeration subAttributes =
										((SimpleAttributeSet) entryValue)
											.getAttributeNames();
									while (subAttributes.hasMoreElements()
										&& currentAnchor.equals("")) {
										Object subKey =
											subAttributes.nextElement();
				...
				parentTextPane.select(caretOffset, caretOffset
					+ internalTextLength);
				...
				if (htmlTag.toString().equals(HTML.Tag.A.toString())) {
					if (!htmlAttribs.containsKey("href")) {
						UserInputAnchorDialog uidInput =
							new UserInputAnchorDialog(parentEkit, Translatrix
					...
					if (htmlAttribs.containsKey("color")) {
						Color color =
							new JColorChooser().showDialog(
								parentEkit.getFrame(), Translatrix
				...
				if (htmlAttribs2.size() > 0) {
					...
					SimpleAttributeSet baseAttrs =
						new SimpleAttributeSet(parentEkit.getTextPane()
							.getCharacterAttributes());
					Enumeration attribEntriesOriginal =
						baseAttrs.getAttributeNames();
	...
	private void insertAttribute(SimpleAttributeSet attrs, Object key,
		 ...
		 * attrs.getAttribute("size")); } else { insertAttr(attrs, "size",
		 * value); } }
		 */
	}
