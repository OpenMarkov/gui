 ...
 */
public class PreferencesEditorDialog extends JDialog implements ActionListener {
	 ...
	 *            owner JFrame
	 */
	public PreferencesEditorDialog(JFrame owner) {
	 ...
	 *            title of dialog
	 */
	public PreferencesEditorDialog(JFrame owner, String title) {
	 ...
	 *            title of dialog
	 * @param userObj
	 *            the package to which this object belongs is used as the
	 ...
	 * 			  if true, then show user preferences
	 * @param systemObj
	 *            the package to which this object belongs is used as the
	 ...
	 */
	public PreferencesEditorDialog(JFrame owner, String title, Object userObj,
	 ...
	 * 
	 * @param userObj
	 *            class to define the User Preferences
	 * @param showUserPrefs
	 *            true if User Preferences will be shown
	 * @param systemObj
	 *            class to define the System Preferences
	 * @param showSystemPrefs
	...
	private MutableTreeNode createUserRootNode(Object obj) {
		...
		try {
			...
			if (obj == null) {
				userRoot = new PreferenceTreeNode(
				                Preferences.userRoot());
			} else {
				userRoot = new PreferenceTreeNode(
				                Preferences.userRoot().node((String)obj));
			}
			return userRoot;
		} catch (BackingStoreException e) {
			e.printStackTrace();
	...
	private MutableTreeNode createSystemNodeForPackage(Object obj) {
		...
		try {
			...
			if (obj == null) {
				...
				systemRoot =
					new PreferenceTreeNode(Preferences
						.systemNodeForPackage((Class<?>) obj));
			}
			return systemRoot;
		} catch (BackingStoreException e) {
			e.printStackTrace();
	...
	protected void actionPerformedCancel() {
		...
		PreferencesTableModel rootPrefTableModel =
			new PreferencesTableModel(rootPreferences);
		rootPrefTableModel.undo();
		Preferences userPreferences = Preferences.userRoot();
		PreferencesTableModel userPrefTableModel =
			new PreferencesTableModel(userPreferences);
	...
	protected void actionPerformedExport() {
		...
		if (chooser.showSaveDialog(PreferencesEditorDialog.this) == JFileChooser.APPROVE_OPTION) {
			try {
				OutputStream out =
	...
	protected void actionPerformedImport() {
		...
		if (chooser.showOpenDialog( PreferencesEditorDialog.this ) == JFileChooser.APPROVE_OPTION) {
			try {
				InputStream in =
		...
		public void valueChanged(TreeSelectionEvent e) {
			...
			try {
				PreferenceTreeNode node =
					(PreferenceTreeNode) e.getPath().getLastPathComponent();
