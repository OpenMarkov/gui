...
public class DocumentWordTokenizer implements WordTokenizer {
	...
	private static int getNextWordStart(Segment text, int startPos) {
		...
		if (startPos <= text.getEndIndex())
			for (char ch = text.setIndex(startPos); ch != Segment.DONE; ch =
				text.next()) {
	...
	public String nextWord() {
		...
		try {
			word =
				document.getText(currentWordPos, currentWordEnd
					- currentWordPos);
		} catch (BadLocationException ex) {
			moreTokens = false;
		}
	...
	public void replaceWord(String newWord) {
		...
		if (currentWordPos != -1) {
			try {
				/*
				 * ORIGINAL document.remove(currentWordPos, currentWordEnd -
				 * currentWordPos); document.insertString(currentWordPos,
				 * newWord, null);
				 */
				...
				document
					.remove(currentWordPos, currentWordEnd - currentWordPos);
