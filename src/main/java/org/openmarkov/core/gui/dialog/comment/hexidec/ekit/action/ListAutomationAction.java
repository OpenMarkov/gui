		...
		try {
			...
			if (selTextBase == null || textLength < 1) {
				...
				if (ae.getActionCommand() != "newListPoint") {
					if (htmlUtilities.checkParentsTag(HTML.Tag.OL)
						...
						new SimpleInfoDialog(
							...
							true,
							Translatrix
								.getTranslationString("ErrorNestedListsNotSupported"));
						return;
					}
				}
				...
				if (htmlUtilities.checkParentsTag(baseTag)) {
					sbNew.append("<li></li>");
					insertHTML(
						parentEkit.getTextPane(), parentEkit
							.getExtendedHtmlDoc(), parentEkit.getTextPane()
							.getCaretPosition(), sbNew.toString(), 0, 0,
						HTML.Tag.LI);
				} else {
					sbNew.append("<" + sListType + "><li></li></" + sListType
						+ "><p>&nbsp;</p>");
					insertHTML(parentEkit.getTextPane(), parentEkit
						.getExtendedHtmlDoc(), parentEkit.getTextPane()
						.getCaretPosition(), sbNew.toString(), 0, 0, (sListType
						.equals("ol") ? HTML.Tag.OL : HTML.Tag.UL));
				}
				...
				StringTokenizer stTokenizer =
					new StringTokenizer(selText, sToken);
				...
				insertHTML(
					jepEditor, htmlDoc, iStart, sbNew.toString(), 1, 1, null);
			}
