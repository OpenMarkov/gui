        ...
        ActionListener
{
    ...
    public void tree_treeExpanded (TreeExpansionEvent event)
    {
        ...
        if (!model.isLeaf (tn1))
        {// the object is a treeADDPotential
     ...
     */
    public void tree_treeWillCollapse (TreeExpansionEvent event)
        throws ExpandVetoException
    {
    ...
    protected void setPopupItemBranch (MouseEvent e, TreeADDBranch branch, TreePath branchPath)
    {
        ...
        if (branch.getPotential () instanceof TablePotential
            || branch.getPotential () instanceof UniformPotential)
        {
        ...
        { // if {variables}-{topVariable}-{conditionedVariable}
    ...
    private void actionChangeInterval (ActionEvent ae, TreeADDBranch branch, TreePath path)
    {
        ...
        if (dialog.requestValues () == ChangeIntervalDialog.OK_BUTTON)
        {
            ...
            // limit situations
            if (!(minText.floatValue () >= minDomainLimit.floatValue () && minText.floatValue () <= branch.getMaxThreshold ().getLimit ())
                || !(maxText.floatValue () >= branch.getMinThreshold ().getLimit () && maxText.floatValue () <= maxDomainLimit.floatValue ()))
            {
            ...
            {
                // max limit
                if (!(maxText.floatValue () == branch.getMaxThreshold ().getLimit () && maxBelongsToLeft == branch.getMaxThreshold ().belongsToLeft ()))
                {
                    ArrayList<TreeADDBranch> followingBranches = new ArrayList<TreeADDBranch> ();
                    parentBranches.get (branchToChangeIndex).setThresholdMax (new Threshold (
                                                                                             maxText,
                                                                                             maxBelongsToLeft));
                    if (branchToChangeIndex != parentBranches.size ())
                    {// branch selected to change interval is not the last one
                        ...
                        for (int i = 0; i < followingBranches.size (); i++)
                        {
                                     ...
                                     && !maxBelongsToLeft)
                            {
                                ...
                                if (i + branchToChangeIndex + 1 <= parentBranches.size ())
                                {
                                    parentBranches.get (branchToChangeIndex + 1).setThresholdMin (new Threshold (
                                                                                                                 maxText,
                                                                                                                 false));
                                }
                                break;
                ...
                // min limit
                if (!(minText.floatValue () == branch.getMinThreshold ().getLimit () && minBelongsToLeft == branch.getMinThreshold ().belongsToLeft ()))
                {
                    ArrayList<TreeADDBranch> previousBranches = new ArrayList<TreeADDBranch> ();
                    parentBranches.get (branchToChangeIndex).setThresholdMin (new Threshold (
                                                                                             minText,
                                                                                             minBelongsToLeft));
                    if (branchToChangeIndex != 0)
                    {// branch selected to change interval is not the first one
                    ...
                    for (int i = 0; i < previousBranches.size (); i++)
                    {
                        ...
                        else if (minText.floatValue () > ((Float) previousBranches.get (i).getMinThreshold ().getLimit ()).floatValue ())
                        {
                            ...
                            aux.get (auxIndex + 1).setThresholdMax (new Threshold (minText,
                                                                                   minBelongsToLeft));
                            break;
                        }
                    }
    ...
    private void actionSplitInterval (ActionEvent ae, TreeADDBranch branch, TreePath path)
    {
        ...
        if (dialog.requestValues () == SplitIntervalDialog.OK_BUTTON)
        {
            ...
            UniformPotential potential = new UniformPotential (
                                                               potentialVariables,
                     ...
                     && maxSecondInterval.belongsToLeft () && belongsToLeft)
            {
                JOptionPane.showMessageDialog (this.getParent (), "This is not a valid action");
                     ...
                     && !minFirstInterval.belongsToLeft () && belongsToLeft)
            {
                TreeADDBranch newBranch = new TreeADDBranch (
                                                             minFirstInterval,
                                                             new Threshold (
                                                                            minFirstInterval.getLimit (),
                                                                            true), potential,
                                                                            branch.getTopVariable (),
            ...
            {// a message to indicate that is not a permitted value
                JOptionPane.showMessageDialog (this.getParent (),
     ...
     * @param path
     */
    private void actionChangeTopVariable (ActionEvent ae, TreeADDPotential treeADDPotential, TreePath path)
    {
        ...
        else if (newTopVariable.getVariableType () == VariableType.NUMERIC)
        {
            ...
            boolean minBelongsToLeftDomain = !isLeftClosed;
            boolean maxBelongsToLeftDomain = isRightClosed;
            newBranches.add (new TreeADDBranch (new Threshold (minDomainLimit,
                                                               minBelongsToLeftDomain),
                                                new Threshold (maxDomainLimit,
                                                               maxBelongsToLeftDomain), potential,
                                                newTopVariable, variables));
        }
                ...
                while (parentPath.getLastPathComponent () != rootTreeADDPotential)
                {
                    ...
                    if (grandParentPath.getLastPathComponent () instanceof TreeADDBranch)
                    {
                        ...
                        if (treeADDBranch.getTopVariable () == newTopVariable)
                        {
                            ...
                            partitionedInterval = new PartitionedInterval (
                                                                           min.belongsToLeft (),
                                                                           (double) min.getLimit (),
                                                                           (double) max.getLimit (),
                                                                           max.belongsToLeft ());
    ...
    private void actionEditPotential (ActionEvent ae, TreeADDBranch branch, TreePath path)
    {
        ...
        PotentialEditDialog dialog = new PotentialEditDialog (Utilities.getOwner (this), dummy,
                                                              false);
    ...
    @SuppressWarnings("unused")
    private void actionAddNewTreeTopVariable (ActionEvent ae,
                                              Object branch,
                                              TreePath path,
                                              Object parentTreeBranch)
    {
        if (!(branch instanceof TreeADDBranch) || !(parentTreeBranch instanceof TreeADDPotential))
        {
            throw new RuntimeException ("Expected TreeADDBranch class, found: "
                                        + branch.getClass ().getName ()
                                        + "Expected TreeADDPotential class, found: "
     ...
     */
    class TreeADDViewer_treeExpansionAdapter
        implements
     ...
     * TODO: Convert to Inner Class of the Viewer?
     */
    class TreeADDViewer_treeWillExpandAdapter
        implements
