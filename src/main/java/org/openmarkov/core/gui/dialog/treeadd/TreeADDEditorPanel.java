...
public class TreeADDEditorPanel extends JScrollPane implements ActionListener {
    ...
    protected void setPopupItemBranch(MouseEvent e, TreeADDBranch branch, TreePath branchPath) {
        ...
        if (possibleVariables.size() != 0 && !(branch.getPotential() instanceof TreeADDPotential)) { // if
                                                                                                     // {variables}-{topVariable}-{conditionedVariable}
                                                                                                     // is
                                                                                                     // not
                                                                                                     // empty
                                                                                                     // so
        ...
        if (branch.getTopVariable().getVariableType() == VariableType.FINITE_STATES) {
            ...
            // Change interval
            Float minDomainLimit = (float) branch.getTopVariable().getPartitionedInterval()
                    .getMin();
            Float maxDomainLimit = (float) branch.getTopVariable().getPartitionedInterval()
                    .getMax();
