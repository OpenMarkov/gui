/*
 * GNU Lesser General Public License FontSelectorDialog Copyright (C) 2003
...
public class FontSelectorDialog extends JDialog implements ItemListener {
	...
	public FontSelectorDialog(Frame parent, String title, boolean bModal,
								String attribName, String demoText) {
		...
		for (int i = 0; i < fonts.length; i++) {
			if (!fonts[i].equals("Dialog") && !fonts[i].equals("DialogInput")
				...
				&& !fonts[i].equals("SansSerif") && !fonts[i].equals("Serif")) {
		...
		final HTMLDocument docFontPreview =
			(HTMLDocument) (kitFontPreview.createDefaultDocument());
		...
		Object[] panelContents =
			{ attribName, jcmbFontlist,
				Translatrix.getTranslationString("FontSample"), jtpFontPreview };
		final Object[] buttonLabels =
			{ Translatrix.getTranslationString("DialogAccept"),
				Translatrix.getTranslationString("DialogCancel") };

		jOptionPane =
			new JOptionPane(panelContents, JOptionPane.QUESTION_MESSAGE,
	...
	private String getFontSampleString(String demoText) {
		...
		return "<HTML><BODY><FONT FACE=" + '"' + jcmbFontlist.getSelectedItem()
			+ '"' + ">" + demoText + "</FONT></BODY></HTML>";
	}
