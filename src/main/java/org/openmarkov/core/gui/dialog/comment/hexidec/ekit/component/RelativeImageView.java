/*
 * GNU Lesser General Public License RelativeImageView Copyright (C) 2001 Frits
 ...
 * @version 1.0 This code was modeled after an artice on <a
...
public class RelativeImageView extends View implements ImageObserver,
	...
	private boolean bLoading; // set to true while the receiver is locked, to
	...
	private void initialize(Element elem) {
		...
		try {
			...
			if (isURL()) {
				...
				if (src != null) {
					Dictionary cache =
						(Dictionary) getDocument().getProperty(
	...
	private String processSrcPath(String src) {
		...
		if (doc != null) {
			...
			if (pv != null) {
				File f = new File(pv);
				val =
					(new File(f.getParent(), imageFile.getPath().toString()))
						.toString();
				found = true;
			}
		}
	...
	private void waitForImage() throws InterruptedException {
		...
		while (true) {
			int flags =
				Toolkit.getDefaultToolkit().checkImage(fImage, w, h, this);
	...
	boolean isLink() {
		...
		AttributeSet anchorAttr =
			(AttributeSet) fElement.getAttributes().getAttribute(HTML.Tag.A);
		if (anchorAttr != null) {
	...
	boolean hasPixels(ImageObserver obs) {
		...
		return ((fImage != null) && (fImage.getHeight(obs) > 0) && (fImage
			.getWidth(obs) > 0));
	}
	...
	public void paint(Graphics g, Shape a) {
		...
		if (!hasPixels(this)) {
			...
			Icon icon =
				((fImage == null) ? sMissingImageIcon : sPendingImageIcon);
			if (icon != null) {
	...
	protected void repaint(long delay) {
		...
		if ((fContainer != null) && (fBounds != null)) {
			fContainer.repaint(
				delay, fBounds.x, fBounds.y, fBounds.width, fBounds.height);
		}
	}
	...
	protected boolean isEditable() {
		...
		return ((fContainer instanceof JEditorPane) && ((JEditorPane) fContainer)
			.isEditable());
	}
	...
	public float getPreferredSpan(int axis) {
		...
		default:
			throw new IllegalArgumentException(
				"Invalid axis in getPreferredSpan() : " + axis);
		}
	}
	...
	public void mousePressed(MouseEvent e) {
		...
		if ((e.getX() >= (size.width - 7)) && (e.getY() >= (size.height - 7))
			...
			fGrowBase =
				new Point(loc.x + e.getX() - fWidth, loc.y + e.getY() - fHeight);
	...
	private Icon makeIcon(final String gifFile) throws IOException {
		...
		InputStream resource =
			RelativeImageView.class.getResourceAsStream(gifFile);
