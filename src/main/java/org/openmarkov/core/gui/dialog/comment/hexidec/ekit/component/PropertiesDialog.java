/*
 * GNU Lesser General Public License PropertiesDialog Copyright (C) 2003 Howard
...
public class PropertiesDialog extends JDialog {
	...
	public PropertiesDialog(Frame parent, String[] fields, String[] types,
							String[] values, String title, boolean bModal) {
		...
		for (int iter = 0; iter < fields.length; iter++) {
			...
			if (fieldType.equals("text")) {
				...
				if (values[iter] != null) {
					((JCheckBox) (fieldComponent))
						.setSelected(values[iter] == "true");
				}
			} else if (fieldType.equals("combo")) {
				fieldComponent = new JComboBox();
				if (values[iter] != null) {
					StringTokenizer stParse =
						new StringTokenizer(values[iter], ",", false);
					while (stParse.hasMoreTokens()) {
						((JComboBox) (fieldComponent)).addItem(stParse
							.nextToken());
					}
				}
			} else {
