...
public class TablePotentialValueEdit extends SimplePNEdit
{
                                    ...
                                    Object[][] notEditablePositions)
    {
        ...
        this.tablePotential = DiscretePotentialOperations.reorder (oldTablePotential,
                                                                   newOrderVariables);
        ...
        throws DoEditException
    {
            ...
            || probNode.getNodeType () == NodeType.DECISION)
        {
            ...
            newTable[potentialSelected] = UtilStrings.roundAndReduce (newValue, epsilon,
                                                                      maxDecimals);
            while (listIterator.hasNext () == true)
            {
                ...
                {
                    sum = UtilStrings.roundAndReduce (sum + newTable[position], epsilon,
                                                      maxDecimals);
                }
                // sum += newTable[pos];
            }
            ...
            if (sum > 1.0)
            {
                ...
                while (listIterator.hasNext () == true && rest != 0)
                {
                    ...
                    {
                        rest = UtilStrings.roundAndReduce (rest - newTable[position], epsilon,
                                                           maxDecimals);
                        // rest = rest - newTable[pos];
                        if (rest < 0)
                        {// it is because the value of the table
                         // is
                         // bigger than the rest
                            // and now there's nothing left to reach one
                            newTable[position] = Math.abs (UtilStrings.roundAndReduce (rest,
                                                                                       epsilon,
                                                                                       maxDecimals));
                            break;
                        }
                        else newTable[position] = 0.0;
                    }
                }
            }
            else
            {// =< 1
                ...
                while (listIterator.hasNext () == true && !updated)
                {
                    ...
                    if (this.isEditablePosition (position))
                    {
                        newTable[position] = UtilStrings.roundAndReduce (newTable[position] + rest,
                                                                         epsilon, maxDecimals);
                        updated = true;
                    }
                }
    ...
    private LinkedList<Integer> getPriorityListInitialization ()
    {
        ...
        {
            if (i != indexSelected) priorityList.add (i + increment);
        }
    ...
    {
        return UtilStrings.toPositionOnJtable (position, col,
                                               probNode.getVariable ().getNumStates (),
                                               probNode.getNode ().getNumParents ());
    }
