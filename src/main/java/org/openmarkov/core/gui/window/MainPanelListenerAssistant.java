        ...
        PropertyNames
{
    ...
    private boolean networkCanBeClosed (NetworkPanel networkPanel)
    {
        ...
        if (networkPanel.getModified ())
        {
            ...
            String message = stringDatabase.getFormattedString ("NetworkNotSaved.Text.Label",
                                                       networkPanel.getTitle ());
            response = JOptionPane.showConfirmDialog (Utilities.getOwner (mainPanel), message,
     ...
     * @param contentPanel content panel of the frame that is trying to be
     *            closed.
                                        ...
                                        SaveOptions saveOptions)
    {
        ...
        try
        {
            ...
            NetsIO.saveNetworkFile (networkPanel.getProbNet (),
                                    networkPanel.getEditorPanel ().getEvidence (), fileName);
    ...
    private boolean saveNetworkAs (NetworkPanel networkPanel)
    {
        ...
        return (fileName != null) ? saveNetworkActions (networkPanel, fileName, saveOptions)
                                 : false;
    }
     ...
     * into the frame.
     * @param network network to be painted into the frame
                    ...
                    for (int j = 0; j < cases[i].length; ++j)
                    {
                        ...
                        try
                        {
                                ...
                                && !variables.get (j).getStateName (cases[i][j]).equals ("?"))
                            {
                                ...
                                try
                                {
                                    newEvidenceCase.addFinding (new Finding (
                                                                             variable,
     ...
     * @param undoOperation - if true, an undo must be performed; if false, a
     *            redo will be performed.
     ...
     * @param newState new edition state to set.
     */
     ...
     * @param newWorkingMode new working mode to set in the network.
     */
     ...
     * @param dialogBox if true, the parameter 'value' is ignored and this value
     *            is requested to user.
     * @param frameContentPanel network whose zoom will be changed.
