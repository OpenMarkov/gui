        ...
        MouseMotionListener
{
                                              ...
                                              boolean isTemporalEvolution)
    {
        ...
        {
            costEffectivenessDialog = new CostEffectivenessDialog (owner, numericTemporalNodes,
                                                                   isThereNodeAge, isUtility,
        ...
        else
        {
            ...
            // isThereNodeAge);
            costEffectivenessDialog = new CostEffectivenessDialog (owner, numericTemporalNodes,
                                                                   isThereNodeAge, true, false,
                                                                   true);
        }
        // costEffectivenessDialog.showSimulationsNumberElements(isProbabilistic);
        return (costEffectivenessDialog.requestData (probNet.getName (), suffixTypeAnalysis) == CostEffectivenessDialog.OK_BUTTON);
    }
    ...
    public EvidenceCase getEvidenceFromNetwork ()
    {
        ...
        if (probNet.getSpecialTimeDependantNodes ().size () >= 0)
        {
            ...
            for (int i = 0; i < probNet.getSpecialTimeDependantNodes ().size (); i++)
            {
                ...
                {
                    finding = new Finding (
                                           probNet.getSpecialTimeDependantNodes ().get (i).getVariable (),
    ...
    public void temporalEvolution ()
    {
        ...
        if (selectedNode.size () == 1)
        {
            ...
            boolean isUtility = node.getProbNode ().getNodeType () == NodeType.UTILITY ? true
                                                                                      : false;
            if (requestCostEffectiveness (Utilities.getOwner (this), "te", false, isUtility, true))
            {
                ...
                try
                {
                    ...
                    new TraceTemporalEvolutionDialog (
                                                      ...
                                                      costEffectivenessDialog,
                                                      variableOfInterest,
                                                      costEffectivenessAnalysis.getExpandedNetwork (),
                                                      isUtility, checkZeroCycle);
                }
