...
/**
 * This class is used for painting and colouring the ICItable and the headers
 * 
 * @author jlgozalo
...
public class ICIValuesTableCellRenderer extends ValuesTableCellRenderer{
	...
	public ICIValuesTableCellRenderer(int firstEditableRow,
			boolean[] editableColumns, ICIPotential iciPotential) {
	...
	protected void setCellColors(JTable table, Object value, boolean isSelected,
		...
		if ((column >= ValuesTable.FIRST_EDITABLE_COLUMN)
				& (row < firstEditableRow)) { //headers cells
				...
				if(row == 0){//FIRST ROW
					for (int i = 0; i < acummulativeColumns.length ; i++) {
						...
						if(acummulativeColumns[i-1]<column && column<=acummulativeColumns[i]){
							if(i % 2 == 0){
								setBackground( new Color(220,220,220));
								setForeground( new Color (128,0,64) ); 
							}else {
