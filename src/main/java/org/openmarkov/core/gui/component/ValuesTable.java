...
public class ValuesTable extends KeyTable implements PNUndoableEditListener {
	 ...
	 * String database
	 */
	protected StringDatabase stringDatabase = StringDatabase
			.getUniqueInstance();
	 ...
	 */
	/*
	 * public ValuesTable() { //TODO verify the use of this class in
	 * NodeUtilityTable //call to constructor is null for probNode parameter
	...
	public ValuesTable(ProbNode probNode, ValuesTableModel tableModel,
		...
		this.tablePotential = (TablePotential)probNode.getPotentials().get(0);
		if (modifiable) {
	...
	public boolean[][] getDataModified() {
		if (dataModified == null) {
			dataModified = new boolean[getTableModel().getRowCount()][getTableModel()
					.getColumnCount()];
		}
		/*
	...
	public void setModel(ValuesTableModel newDataModel)
		...
		tableRowSorter = new TableRowSorter<ValuesTableModel>(
		...
		/*
		 * tableRowSorter.setRowFilter( RowFilter.notFilter(
		 * RowFilter.regexFilter( "Edad\\[\\d\\]", 0 ) )); this.setRowSorter(
		 * tableRowSorter);
		 */
	}
	...
	public void changeSelection(int rowIndex, int columnIndex, boolean toggle,
		...
		/*
		 * if (editCellAt(rowIndex, columnIndex)) { Component editor =
		 * getEditorComponent(); ((Component)editor).selectAll();
	...
	public void setValueAt(Object newValue, int row, int col) {
		...
		if (((Double) newValue) < 0
			...
			JOptionPane.showMessageDialog(this.getParent(),
					"Introduced value cannot be negative");
		}
	...
	public void checkProbabilistic(Object oldValue, Object newValue, int row,
		...
		try {
			...
			if (rowsToModified > 0) {
				assignNewProbabilisticValuesToColumn(oldValue, value, row, col,
						rowsToModified);
			}
	...
	private void showNodePotentialTableErrorMsg(String msg) {
		JOptionPane.showMessageDialog(this,
				stringDatabase.getString(msg + ".Text"),
	...
	private int existsRowsToModify(int column) {
		...
		int numOfRowsToModify = countRowsNotModified(column, initialRow,
				lastEditableRow);
		...
		if (((Double) value) == 1.0) {
			for (int i = initialRow; i < lastEditableRow; i++) {
				if (((Double) super.getModel().getValueAt(i, col)) == 1.0) {
					...
					super.getModel().setValueAt(value, row, col);
					String stateValue = (String) super.getModel().getValueAt(
							row, 0);
					super.getModel().setValueAt(stateValue, lastEditableRow,
							col);
					break;
				}
			}
			...
			if (row == initialRow) {
				if (row == lastEditableRow) {
					...
					stateValue = (String) super.getModel().getValueAt(row + 1,
							0);
				}
			} else {
				super.getModel().setValueAt(1.0, row - 1, col);
	...
	private void assignNewProbabilisticValuesToColumn(Object oldValue,
			Object value, int row, int col, int numRowsToModify) {
		int initialRow = ((ValuesTableModel) getModel()).getFirstEditableRow();
		double delta = 0.0;
		double summa = 0.0;
		double newSumma = 0.0;
		...
		for (int i = initialRow; i < lastEditableRow; i++) {
			if (dataModified[i][col]) { // summarize all previous edited values
				summa = roundingDouble(summa
						+ (Double) super.getModel().getValueAt(i, col));
			}
		}
		...
		if (newSumma > 1.0) {
			...
			if (numRowsToModify != 0) {
				delta = ((((Double) value).doubleValue() - ((Double) oldValue)
				...
				for (int i = initialRow; i < lastEditableRow; i++) {
					if (i != row && !dataModified[i][col]) {
						...
						if (newSumma > 1.0) {
							...
							auxValue = roundingDouble(auxValue
									- (newSumma - 1.0));
						}
	...
	public void setUsingGeneralPotential(int indexPotential) {
		...
		if (isUsingGeneralPotential()) {// if indexPotential == 0
			...
			if ("leak".equals(getValueAt(0, getColumnCount() - 1))) {
				...
				int firstRow = ((ValuesTableModel) getModel())
						.getFirstEditableRow();
	...
	public void setDeterministic(boolean deterministic) {
		...
		if (isDeterministic()) {
			for (int k = FIRST_EDITABLE_COLUMN; k < getModel().getColumnCount(); k++) {
				double maxValue = -1;
				int rowPosition = -1;
				for (int i = ((ValuesTableModel) getModel())
						.getFirstEditableRow(); i < getLastEditableRow(); i++) {
					if (((Double) ((ValuesTableModel) getModel()).getValueAt(i,
							k)) > maxValue) {
						maxValue = (Double) ((ValuesTableModel) getModel())
								.getValueAt(i, k);
						rowPosition = i;
					}
				}
	...
	public void setShowingAllParameters(boolean showingAllParameters) {
		...
		if (isShowingAllParameters()) {
			...
			list.add(RowFilter.notFilter(RowFilter.regexFilter(
					(String) getModel().getValueAt(lastRow, 0), 0)));
			list.add(RowFilter.notFilter(RowFilter.regexFilter(getVariable()
					.getName(), 0)));
	...
	public static String[] getColumnsIdsSpreadSheetStyle(int howManyColumns) {
		...
		for (int columnPosition = 0; columnPosition < howManyColumns; columnPosition++) {
			...
			if (columnPosition >= (26 * 27)) {
				...
				columnId = columnId
						+ ALPHABET.substring(secondLetterPosition,
								secondLetterPosition + 1)
						+ ALPHABET.substring(firstLetterPosition,
	...
	private boolean askToResetDataModified() {
		...
		result = JOptionPane
				.showConfirmDialog(
						this,
						this.stringDatabase
								.getString("NodePotentialTable.Msg.AllowAgainValuesToModified.Text"),
						this.stringDatabase
								.getString("NodePotentialTable.Msg.AllowAgainValuesToModified.Title"),
	...
	private void auxUndoableEditHappenedUncertainValuesEdit(
		...
		UncertainValuesEdit edit = (UncertainValuesEdit) (arg0.getEdit());
		isChance = edit.isChanceVariable();
		TablePotential tablePotential = (TablePotential) edit.getProbNode()
				.getPotentials().get(0);
	...
	public void auxUndoableEditHappenedNodePotentialEdit(UndoableEditEvent arg0) {
		int position = 0;
		TablePotentialValueEdit edit = (TablePotentialValueEdit) arg0.getEdit();
		...
		case CHANCE:
			...
			List<Variable> orderVariables = probNode.getPotentials().get(0)
					.getVariables();
	...
	public void undoEditHappened(UndoableEditEvent event) {
		...
		if (event.getEdit() instanceof TablePotentialValueEdit) {
			...
			case CHANCE:
				priorityList = edit.getPriorityList();
				ListIterator<Integer> listIterator = priorityList
						.listIterator();
				while (listIterator.hasNext() == true) {
					position = (Integer) listIterator.next();
					super.getModel().setValueAt(
							tablePotential.values[position],
							edit.getRowPosition(position),
							edit.getColumnPosition());
				}
				break;
	...
	public void setData(ProbNode probNode) {
		if (this.probNode.getProbNet().getPNESupport() != probNode.getProbNet()
				.getPNESupport()) {
			this.probNode.getProbNet().getPNESupport()
