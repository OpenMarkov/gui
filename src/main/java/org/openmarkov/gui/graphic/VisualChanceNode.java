...
public non-sealed class VisualChanceNode extends VisualNode {
     ...
     * and 5).
     */
    public double[] getCenteredPoints(Graphics2D g) {
        ...
        //new RoundRectangle2D.Double(getTemporalPosition().getX(), getTemporalPosition().getY(), width, height, ARC_WIDTH, ARC_HEIGHT);
     ...
     */
    @Override public RoundRectangle2D.Double getCenteredShape(Graphics2D g) {
        double[] dimensions = getCenteredPoints(g);
    ...
    @Override public void paint(Graphics2D g) {
        ...
        if (isExpanded()) {
            text = adjustText(text, shape.getBounds2D().getWidth(), 3, FONT_HELVETICA, g);
        }
