 ...
 */
public non-sealed class VisualDecisionNode extends VisualNode {
     ...
     */
    public Point2D.Double[] getPoints(Graphics2D g) {
     ...
     */
    @Override public Rectangle2D.Double getCenteredShape(Graphics2D g) {
        Point2D.Double[] points = getPoints(g);
    ...
    @Override public Rectangle2D.Double getShape(Graphics2D g) {
        Point2D.Double[] points = getPoints(g);
        double rectangleWidth = points[0].distance(points[1]);
        double rectangleHeight = points[1].distance(points[2]);
        double rectanglePosX = getTemporalPosition().getX();
        double rectanglePosY = getTemporalPosition().getY();
        return new Rectangle2D.Double(rectanglePosX, rectanglePosY, rectangleWidth, rectangleHeight);
    }
    ...
    @Override public Point2D.Double getCutPoint(Segment segment, Graphics2D g) {
        ...
        Rectangle2D.Double shape = getShape(g);
