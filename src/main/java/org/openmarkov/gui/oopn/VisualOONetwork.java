...
import org.openmarkov.gui.graphic.SelectionRectangle;
...
public class VisualOONetwork extends VisualNetwork {
     ...
     */
    @Override protected void constructVisualInfo() {
        super.constructVisualInfo();
        if (!(probNet instanceof OOPNet && getWorkingMode() == NetworkPanel.WorkingMode.EDITION)) {
            return;
        }
        ...
        for (String instanceName : ((OOPNet) probNet).getInstances().keySet()) {
            visualInstances.put(instanceName,
                                new VisualInstance(((OOPNet) probNet).getInstances()
                                                                     .get(instanceName), visualNodes));
        }
        ...
        for (ReferenceLink link : ((OOPNet) probNet).getReferenceLinks()) {
            if (link instanceof InstanceReferenceLink instanceLink) {
                ...
                VisualInstance destVisualInstance = visualInstances.get(instanceLink.getDestInstance().getName())
                                                                   .getSubInstance(instanceLink.getDestSubInstance()
                                                                                               .getName());
        ...
        for (VisualLink visualLink : visualLinks) {
            if (contractedNodes.containsKey(visualLink.getSourceNode())) {
                ...
                if (!contractedNodes.containsKey(visualLink.getDestinationNode())) {
                    visualContractedNodeLinks
                            .add(new VisualContractedNodeLink(contractedNodes.get(visualLink.getSourceNode()),
    ...
    @Override protected void notifyObjectsSelected() {
        ...
        for (SelectionListener listener : selectionListeners) {
            ...
            if (listener instanceof OOSelectionListener) {
                ((OOSelectionListener) listener)
                        .objectsSelected(getSelectedNodes(), getSelectedLinks(), getSelectedInstances(),
                                         getSelectedReferenceLinks());
            }
        }
    }
     ...
     *
     * @param instance Visual instance to be selected/deselected.
     * @param selected new selection state.
     */
     ...
     * @param selected new selection state.
     */
    @Override public void setSelectedAllObjects(boolean selected) {
     ...
     */
    @Override public void selectElementsInsideSelection(SelectionRectangle selection) {
        ...
        for (VisualInstance instance : visualInstances.values()) {
            if (selection.containsRectangle(instance.getCoordinateX(), instance.getCoordinateY(), instance.getWidth(),
                                            instance.getHeight())) {
    ...
    @Override public void startLinkCreation(java.awt.geom.Point2D.Double cursorPosition, Graphics2D g) {
        ...
        if ((node = whatNodeInPosition(cursorPosition, g)) != null) {
            ...
            newLink = new VisualArrow(new Point2D.Double(instance.getCenter().getX(), instance.getCenter().getY()),
                                      cursorPosition);
    ...
    @Override public PNEdit finishLinkCreation(java.awt.geom.Point2D.Double point, Graphics2D g) {
        ...
        if ((newInstanceLinkDestination = getInstanceInPosition(point, g)) != null && newInstanceLinkSource != null) {
            newLink = null;
            VisualInstance inputParameter = newInstanceLinkDestination.getParameterInPosition(point, g);
            if (inputParameter != null && inputParameter.getInstance().getClassNet().getName()
                                                        .compareToIgnoreCase(newInstanceLinkSource.getInstance()
                                                                                                  .getClassNet()
                                                                                                  .getName()) == 0) {
                linkEdit = new AddReferenceLinkEdit(probNet, newInstanceLinkSource.getInstance(),
                                                    newInstanceLinkDestination.getInstance(), inputParameter.getInstance());
            }
        } else if ((newLinkDestination = whatNodeInPosition(point, g)) != null && newLinkSource != null) {
            if (newLinkDestination.getNode().isInput() && isEquivalentVariable(
                    newLinkDestination.getNode().getVariable(), newLinkSource.getNode().getVariable())) {
                newLink = null;
    ...
    @Override public void editInstanceName() throws DoEditException {
        Instance selectedInstance = ((VisualInstance) selectedInstances.toArray()[0]).getInstance();
    ...
    @Override public void setWorkingMode(NetworkPanel.WorkingMode workingMode) {
        ...
        switch (workingMode) {
            case EDITION -> constructVisualInfo();
            case INFERENCE -> {
                visualInstances.clear();
                visualReferenceLinks.clear();
                visualContractedNodeLinks.clear();
