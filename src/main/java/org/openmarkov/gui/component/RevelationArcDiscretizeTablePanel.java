		...
		if (arg0.getType() == TableModelEvent.UPDATE && ((DiscretizeTableModel) arg0.getSource())
			...
			if (lower) {
				double upperLimit = (Double) (
						(DiscretizeTableModel) arg0.getSource()
				).getValueAt(row, UPPER_BOUND_VALUE_COLUMN_INDEX);
				if (upperLimit < newValue) {
					JOptionPane.showMessageDialog(this, stringDatabase.getString("IntervalInconsistent.Text.Label"),
							stringDatabase.getString("IntervalEditError.Text.Label"), JOptionPane.ERROR_MESSAGE);
				}

			} else {
				double lowerLimit = (Double) (
						(DiscretizeTableModel) arg0.getSource()
				).getValueAt(row, LOWER_BOUND_VALUE_COLUMN_INDEX);
			...
			if (lower && row > 0) {
				double previousLimit = (Double) (
						(DiscretizeTableModel) arg0.getSource()
				).getValueAt(row - 1, UPPER_BOUND_VALUE_COLUMN_INDEX);
				if (previousLimit > newValue)
					JOptionPane.showMessageDialog(this, stringDatabase.getString("IntervalOverlap.Text.Label"),
							stringDatabase.getString("IntervalEditError.Text.Label"), JOptionPane.ERROR_MESSAGE);

			}
			if (!lower && row < numRows) {
				double nextLimit = (Double) (
						(DiscretizeTableModel) arg0.getSource()
				).getValueAt(row + 1, LOWER_BOUND_VALUE_COLUMN_INDEX);
		...
		if (column == LOWER_BOUND_SYMBOL_COLUMN_INDEX || column == UPPER_BOUND_SYMBOL_COLUMN_INDEX) {
			...
			if (column == LOWER_BOUND_SYMBOL_COLUMN_INDEX) {
				...
				if (aux.equals("(")) {
					...
					if (fila > 0) {
						Double lowerLimit = (Double) valuesTable.getValueAt(fila, LOWER_BOUND_VALUE_COLUMN_INDEX);
						Double upperLimit = (Double) valuesTable.getValueAt(fila - 1, UPPER_BOUND_VALUE_COLUMN_INDEX);
						if (lowerLimit.equals(upperLimit))
							valuesTable.setValueAt(")", fila - 1, UPPER_BOUND_SYMBOL_COLUMN_INDEX);
			...
			if (column == UPPER_BOUND_SYMBOL_COLUMN_INDEX) {
				if (aux.equals(")")) {
					...
					if (fila < valuesTable.getRowCount() - 1) {
						Double lowerLimit = (Double) valuesTable.getValueAt(fila, UPPER_BOUND_VALUE_COLUMN_INDEX);
						Double upperLimit = (Double) valuesTable.getValueAt(fila + 1, LOWER_BOUND_VALUE_COLUMN_INDEX);
						if (lowerLimit.equals(upperLimit))
							valuesTable.setValueAt("(", fila + 1, LOWER_BOUND_SYMBOL_COLUMN_INDEX);
						relatedIntervalEdit = new RevelationIntervalEdit(link, StateAction.MODIFY_DELIMITER_INTERVAL,
								fila + 1, 0, true);
					}
					// checkIntervalDiscretize("]", fila, columna,upMonotony);
