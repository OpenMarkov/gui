...
public class AddFindingDialog extends JDialog implements ActionListener
{
     ...
     */
    public AddFindingDialog (Window owner,
                                 ...
                                 EditorPanel editorPanel)
    {
        ...
        }else
        {
        	...
        	double minValue = (variableDomain.isLeftClosed())? variableDomain.getMin() : variableDomain.getMin() + variable.getPrecision();
        	double maxValue = (variableDomain.isRightClosed())? variableDomain.getMax() : variableDomain.getMax() - variable.getPrecision();
        	double defaultValue = (finding != null)? finding.getNumericalValue() : minValue;
        	SpinnerNumberModel model = new SpinnerNumberModel(defaultValue, minValue, maxValue, variable.getPrecision());
        ...
        try {
	        if (command.equals (stringDatabase.getString ("AddFindingDialog.OKButton.Label")))
	        {
	        	...
	        	if(variable.getVariableType() == VariableType.FINITE_STATES)
	            {
					...
					editorPanel.setNewFinding (visualNode, new Finding(variable, variable.getState(selectedState)), false);
	            }else
	            {
