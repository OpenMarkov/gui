...
@PotentialPanelPlugin(potentialType = "Binomial")
public class BinomialPotentialPanel extends PotentialPanel {
    ...
    public void setData(Node node) {
        ...
        if (!node.getPotentials().isEmpty()
                && node.getPotentials().get(0) instanceof BinomialPotential)
        {
    ...
    public boolean saveChanges() {
        ...
        if (node.getVariable().getVariableType() == VariableType.NUMERIC){
        	...
        	if (( (NValue>0) && (NValue<= Integer.MAX_VALUE)) && ((thetaValue>=0)&&(thetaValue<=1.00))){
        		...
        		newPotential = new BinomialPotential(oldPotential.getVariables(), oldPotential.getPotentialRole(), NValue,thetaValue);
        	}else{
        		JOptionPane.showMessageDialog(null, "The entered values are out of bounds. The Number of cases 'N' must be a positive integer and the probability 'theta' must be between 0 and 1",
