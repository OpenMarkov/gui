...
@PotentialPanelPlugin(potentialType = "Conditional Gaussian")
public class ConditionalGaussianPotentialPanel extends PotentialPanel implements PNUndoableEditListener{
	...
	private void editMeanPotential() {
		PotentialEditDialog potentialEditDialog = new PotentialEditDialog(Utilities.getOwner (this), meanDummyNode, false, isReadOnly());
	...
	private void editVariancePotential() {
		PotentialEditDialog potentialEditDialog = new PotentialEditDialog(Utilities.getOwner (this), varianceDummyNode, false, isReadOnly());
		if (potentialEditDialog.requestValues() == NodePropertiesDialog.OK_BUTTON)
		{
			// Do nothing?
		}else
