...
public class TreeADDEditorPanel extends JScrollPane implements ActionListener {
	...
	private void changeInterval(ActionEvent ae, TreeADDBranch branch, TreePath path) {
		...
		if (dialog.requestValues() == ChangeIntervalDialog.OK_BUTTON) {
			...
			} else if (upperBound.floatValue() > maxDomainLimit || lowerBound.floatValue() < minDomainLimit) {
				JOptionPane
						.showMessageDialog(this.getParent(), stringDatabase.getString("TreeADD.DomainChangeWarning"));
	...
	private void splitInterval(ActionEvent ae, TreeADDBranch branch, TreePath path) {
		...
		if (dialog.requestValues() == SplitIntervalDialog.OK_BUTTON) {
			...
			boolean minBelongsToLeftDomain = !isLeftClosed;
			boolean maxBelongsToLeftDomain = isRightClosed;
			if (minDomainLimit == minFirstInterval.getLimit() && introducedLimit.floatValue() == minFirstInterval
					.getLimit() && minBelongsToLeftDomain && belongsToLeft) {
				JOptionPane.showMessageDialog(this.getParent(), "Be careful, you are trying to change variable domain");
					...
					&& introducedLimit.floatValue() == maxSecondInterval.getLimit() && !maxBelongsToLeftDomain
					&& !belongsToLeft) {
				JOptionPane.showMessageDialog(this.getParent(), "Be careful, you are trying to change variable domain");
			} else if (minFirstInterval.getLimit() == introducedLimit.floatValue() && minFirstInterval.belongsToLeft()
					&& belongsToLeft) {
				JOptionPane.showMessageDialog(this.getParent(), "This is not a valid action");
			} else if (minFirstInterval.getLimit() == introducedLimit.floatValue() && !minFirstInterval.belongsToLeft()
					&& !belongsToLeft) {
				JOptionPane.showMessageDialog(this.getParent(), "This is not a valid action");
			} else if (maxSecondInterval.getLimit() == introducedLimit.floatValue() && !maxSecondInterval
					.belongsToLeft() && !belongsToLeft) {
				JOptionPane.showMessageDialog(this.getParent(), "This is not a valid action");
			} else if (maxSecondInterval.getLimit() == introducedLimit.floatValue() && maxSecondInterval.belongsToLeft()
					&& belongsToLeft) {
				JOptionPane.showMessageDialog(this.getParent(), "This is not a valid action");
			} else if (minFirstInterval.getLimit() == introducedLimit.floatValue() && !minFirstInterval.belongsToLeft()
					&& belongsToLeft) {
				...
				TreeADDBranch newBranch = new TreeADDBranch(new Threshold(maxSecondInterval.getLimit(), false),
				...
				TreeADDBranch firstBranch = new TreeADDBranch(minFirstInterval, maxFirstInterval,
						branch.getRootVariable(), branch.getPotential().copy(), branch.getParentVariables());
				TreeADDBranch secondBranch = new TreeADDBranch(minSecondInterval, maxSecondInterval,
						branch.getRootVariable(), branch.getPotential().copy(), branch.getParentVariables());
				for (TreeADDBranch parentBranch : parentBranches) {
					if (branch == parentBranch) {
	...
	private void removeSubtree(ActionEvent ae, TreeADDBranch branch, TreePath path) {
        ...
        */
		...
		//carmenyago There is not utilityVariable any more
        /*
	...
	private void dissociateStates(ActionEvent ae, TreeADDBranch branch, TreePath path) {
		...
		if (dialog.requestValues() == RemoveStatesDialog.OK_BUTTON) {
			...
			if (checkBoxes.size() == statesToEliminate.size()) {
				...
				//carmenyago. parentTreeADD is TreeADDPotential.There is not utilityVariable any more
				//
                /*
                if (parentTreeADD.isUtility()) {
	...
	private void addSubtree(ActionEvent ae, TreeADDBranch branch, TreePath path) {
		...
		// carmenyago Now there is not utilityVariable
       /*
